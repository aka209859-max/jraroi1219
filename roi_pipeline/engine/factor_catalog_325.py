...
# =============================================================================
# JRD_KYI (65列) - 競馬指数
#   実テーブル参照: jrd_kyi_fixed（490,149件 2016-2026, 38カラム）
#   jrd_kyi_fixed に存在しないカラムは NOT_IN_DB/SKIP
#
#   jrd_kyi_fixed の利用可能データカラム（23列）:
#     idm, kishu_shisu, sogo_shisu, kyakushitsu, kyori_tekisei,
#     chokyo_shisu, kyusha_shisu, chokyo_yajirushi_code, omo_tekisei_code,
#     shiba_tekisei_code, da_tekisei_code, soho, blinker, futan_juryo,
#     ten_shisu, pace_shisu, agari_shisu, ichi_shisu, kyori_tekisei_2,
#     seibetsu_code, kishu_code, chokyoshi_code, kyusha_rank
# =============================================================================
_KYI = [
    Factor325(210, "jrd_kyi", "idm",                       "kyi_idm",                       "NUMERIC",  "IDM（スピード指数）",       n_bins=10),
    Factor325(211, "jrd_kyi", "joho_shisu",                "kyi_joho_shisu",                "SKIP",     "情報指数",                  "NOT_IN_DB"),
    ...
    Factor325(215, "jrd_kyi", "gekiso_shisu",              "kyi_gekiso_shisu",              "SKIP",     "激走指数",                  "NOT_IN_DB"),
    ...
    Factor325(218, "jrd_kyi", "manken_shisu",              "kyi_manken_shisu",              "SKIP",     "万券指数",                  "NOT_IN_DB"),
    ...
    Factor325(222, "jrd_kyi", "uma_start_shisu",           "kyi_uma_start_shisu",           "SKIP",     "馬スタート指数",            "NOT_IN_DB"),
    Factor325(223, "jrd_kyi", "agari_shisu_juni",          "kyi_agari_shisu_juni",          "SKIP",     "上がり指数順位",            "NOT_IN_DB"),
    Factor325(224, "jrd_kyi", "dochu_juni",                "kyi_dochu_juni",                "SKIP",     "道中順位",                  "NOT_IN_DB"),
    Factor325(225, "jrd_kyi", "dochu_sa",                  "kyi_dochu_sa",                  "SKIP",     "道中差",                    "NOT_IN_DB"),
    Factor325(226, "jrd_kyi", "dochu_uchisoto",            "kyi_dochu_uchisoto",            "SKIP",     "道中内外",                  "NOT_IN_DB"),
    Factor325(227, "jrd_kyi", "gekiso_juni",               "kyi_gekiso_juni",               "SKIP",     "激走順位",                  "NOT_IN_DB"),
    Factor325(228, "jrd_kyi", "goal_juni",                 "kyi_goal_juni",                 "SKIP",     "ゴール順位",                "NOT_IN_DB"),
    Factor325(229, "jrd_kyi", "goal_sa",                   "kyi_goal_sa",                   "SKIP",     "ゴール差",                  "NOT_IN_DB"),
    Factor325(230, "jrd_kyi", "goal_uchisoto",             "kyi_goal_uchisoto",             "SKIP",     "ゴール内外",                "NOT_IN_DB"),
    Factor325(231, "jrd_kyi", "ichi_shisu_juni",           "kyi_ichi_shisu_juni",           "SKIP",     "位置取り指数順位",          "NOT_IN_DB"),
    Factor325(232, "jrd_kyi", "kohan_3f_juni",             "kyi_kohan_3f_juni",             "SKIP",     "後半3F順位",                "NOT_IN_DB"),
    Factor325(233, "jrd_kyi", "kohan_3f_sa",               "kyi_kohan_3f_sa",               "SKIP",     "後半3F差",                  "NOT_IN_DB"),
    Factor325(234, "jrd_kyi", "kohan_3f_uchisoto",         "kyi_kohan_3f_uchisoto",         "SKIP",     "後半3F内外",                "NOT_IN_DB"),
    Factor325(235, "jrd_kyi", "ls_shisu_juni",             "kyi_ls_shisu_juni",             "SKIP",     "LS指数順位",                "NOT_IN_DB"),
    Factor325(236, "jrd_kyi", "pace_shisu_juni",           "kyi_pace_shisu_juni",           "SKIP",     "ペース指数順位",            "NOT_IN_DB"),
    Factor325(237, "jrd_kyi", "ten_shisu_juni",            "kyi_ten_shisu_juni",            "SKIP",     "テン指数順位",              "NOT_IN_DB"),
    Factor325(238, "jrd_kyi", "chokyo_yajirushi_code",     "kyi_chokyo_yajirushi_code",     "CATEGORY", "調教矢印コード"),
    Factor325(239, "jrd_kyi", "class_code",                "kyi_class_code",                "SKIP",     "クラスコード",              "NOT_IN_DB"),
    Factor325(240, "jrd_kyi", "gekiso_type",               "kyi_gekiso_type",               "SKIP",     "激走タイプ",                "NOT_IN_DB"),
    Factor325(241, "jrd_kyi", "hizume_code",               "kyi_hizume_code",               "SKIP",     "蹄コード",                  "NOT_IN_DB"),
    Factor325(242, "jrd_kyi", "hobokusaki",                "kyi_hobokusaki",                "SKIP",     "放牧先",                    "IDENTIFIER"),
    Factor325(243, "jrd_kyi", "hobokusaki_rank",           "kyi_hobokusaki_rank",           "SKIP",     "放牧先ランク",              "NOT_IN_DB"),
    Factor325(244, "jrd_kyi", "joshodo_code",              "kyi_joshodo_code",              "SKIP",     "上昇度コード",              "NOT_IN_DB"),
    Factor325(245, "jrd_kyi", "kishu_code",                "kyi_kishu_code",                "CATEGORY", "騎手コード"),
    Factor325(246, "jrd_kyi", "kyakushitsu",               "kyi_kyakushitsu_code",          "CATEGORY", "脚質コード"),              # jrd_kyi_fixed: kyakushitsu (no _code suffix)
    Factor325(247, "jrd_kyi", "kyori_tekisei",             "kyi_kyori_tekisei_code",        "CATEGORY", "距離適性コード"),           # jrd_kyi_fixed: kyori_tekisei (no _code suffix)
    Factor325(248, "jrd_kyi", "kyusha_hyoka_code",         "kyi_kyusha_hyoka_code",         "SKIP",     "厩舎評価コード",            "NOT_IN_DB"),
    Factor325(249, "jrd_kyi", "kyusha_rank",               "kyi_kyusha_rank",               "CATEGORY", "厩舎ランク"),
    Factor325(250, "jrd_kyi", "kyuyo_riyu_bunrui_code",    "kyi_kyuyo_riyu_bunrui_code",    "SKIP",     "休養理由分類コード",        "NOT_IN_DB"),
    Factor325(251, "jrd_kyi", "manken_shirushi",           "kyi_manken_shirushi",           "SKIP",     "万券印",                    "NOT_IN_DB"),
    Factor325(252, "jrd_kyi", "pace_yoso",                 "kyi_pace_yoso",                 "SKIP",     "ペース予想",                "NOT_IN_DB"),
    Factor325(253, "jrd_kyi", "omo_tekisei_code",          "kyi_tekisei_code_omo",          "CATEGORY", "適性コード（重）"),         # jrd_kyi_fixed: omo_tekisei_code
    Factor325(254, "jrd_kyi", "yuso_kubun",                "kyi_yuso_kubun",                "SKIP",     "輸送区分",                  "NOT_IN_DB"),
    Factor325(255, "jrd_kyi", "kakutoku_shokin_ruikei",    "kyi_kakutoku_shokin_ruikei",    "SKIP",     "獲得賞金累計",              "NOT_IN_DB"),
    Factor325(256, "jrd_kyi", "kijun_ninkijun_fukusho",    "kyi_kijun_ninkijun_fukusho",    "SKIP",     "基準人気順（複勝）",        "NOT_IN_DB"),
    Factor325(257, "jrd_kyi", "kijun_ninkijun_tansho",     "kyi_kijun_ninkijun_tansho",     "SKIP",     "基準人気順（単勝）",        "NOT_IN_DB"),
    Factor325(258, "jrd_kyi", "kijun_odds_fukusho",        "kyi_kijun_odds_fukusho",        "SKIP",     "基準オッズ（複勝）",        "NOT_IN_DB"),
    Factor325(259, "jrd_kyi", "kijun_odds_tansho",         "kyi_kijun_odds_tansho",         "SKIP",     "基準オッズ（単勝）",        "NOT_IN_DB"),
    Factor325(260, "jrd_kyi", "shutoku_shokin_ruikei",     "kyi_shutoku_shokin_ruikei",     "SKIP",     "取得賞金累計",              "NOT_IN_DB"),
    Factor325(261, "jrd_kyi", "kishu_kitai_rentai_ritsu",  "kyi_kishu_kitai_rentai_ritsu",  "SKIP",     "騎手期待連対率",            "NOT_IN_DB"),
    Factor325(262, "jrd_kyi", "kishu_kitai_sanchakunai_ritsu","kyi_kishu_kitai_sanchakunai_ritsu","SKIP","騎手期待3着内率",          "NOT_IN_DB"),
    Factor325(263, "jrd_kyi", "kishu_kitai_tansho_ritsu",  "kyi_kishu_kitai_tansho_ritsu",  "SKIP",     "騎手期待単勝率",            "NOT_IN_DB"),
    Factor325(264, "jrd_kyi", "taikei",                    "kyi_taikei",                    "SKIP",     "体型（テキスト）",          "IDENTIFIER"),
    Factor325(265, "jrd_kyi", "taikei_sogo_1",             "kyi_taikei_sogo_1",             "SKIP",     "体型総合1",                 "NOT_IN_DB"),
    Factor325(266, "jrd_kyi", "taikei_sogo_2",             "kyi_taikei_sogo_2",             "SKIP",     "体型総合2",                 "NOT_IN_DB"),
    Factor325(267, "jrd_kyi", "taikei_sogo_3",             "kyi_taikei_sogo_3",             "SKIP",     "体型総合3",                 "NOT_IN_DB"),
    Factor325(268, "jrd_kyi", "uma_tokki_1",               "kyi_uma_tokki_1",               "SKIP",     "馬特記1",                   "NOT_IN_DB"),
    Factor325(269, "jrd_kyi", "uma_tokki_2",               "kyi_uma_tokki_2",               "SKIP",     "馬特記2",                   "NOT_IN_DB"),
    Factor325(270, "jrd_kyi", "uma_tokki_3",               "kyi_uma_tokki_3",               "SKIP",     "馬特記3",                   "NOT_IN_DB"),
    ...
    Factor325(274, "jrd_kyi", "uma_deokure_ritsu",         "kyi_uma_deokure_ritsu",         "SKIP",     "馬出遅れ率",                "NOT_IN_DB"),
]
