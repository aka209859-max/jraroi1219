...
def filter_edge_table(
    ...
    Args:
        ...
        baseline: LCBゲートの baseline（デフォルト 0.80）
        quantile: LCBゲートの分位点（デフォルト 0.10）
...
def summarize_edge_table(edge_table: Dict) -> Dict:
    ...
    all_scores = [log_ev_score(cell["posterior_mean"] / 100.0) for cell in edge_table.values()]
    pass_keys = [
        k for k, cell in edge_table.items()
        if lcb_gate(cell["posterior_samples"])
    ]
    pass_scores = [log_ev_score(edge_table[k]["posterior_mean"] / 100.0) for k in pass_keys]
