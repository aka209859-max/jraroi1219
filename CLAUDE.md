...
## Phase 3: 統合アーキテクチャ実装（The Unified Pipeline）

### ステータス
- Phase 1: 完了（20ファクター検証、13ファクターにエッジ確認）
- Phase 2: 完了（13ファクター×セグメント交互作用分析、単勝・複勝バグ修正済、114テスト全パス、グローバル補正回収率 79.92%）
- Phase 3: **これから実装**

### Phase 3 概要
Phase 2で確認された13ファクターのエッジを、学術的に最高峰の投資競馬ロジックで得点化・確率推定・資金配分まで一貫したパイプラインとして実装する。

Phase 3は4つのTaskで構成される。詳細仕様は `roi_pipeline/docs/PHASE3_ARCHITECTURE_SPEC.md` を参照すること。
...
### 実行順序
Task 1 → Task 2 → Task 3 → Task 4（順序厳守、各Task完了後にテスト全パスを確認してから次へ）

### 絶対遵守事項
1. Phase 1・Phase 2の既存コード・テストを変更しない
2. 新規ファイルのみ作成する
3. 各Task完了時にテスト全パスを確認する
4. CLAUDE.md の session log に各Task完了ログを追記する
5. 補正回収率の基準値は 80%（0.80）。これがゼロライン。
6. 単勝と複勝は独立に処理し、統合は Task 2 の Benter 二段階モデルで行う
7. 全ての得点化・確率推定において、少標本セグメントは階層ベイズのシュリンケージで自動的にグローバル平均（80%）に収縮させる
